...
struct DataComp
{
    bool operator()(const DataAttr* lhs, const DataAttr* rhs) const
    {
        ...
        else if (lhs->m_neighbors.size() != rhs->m_neighbors.size()) {return lhs->m_neighbors.size() > rhs->m_neighbors.size();}
    ...
    if (read_file.is_open()) {
        ...
        while (std::getline(read_file, data)) {
            ...
            if (offset == BLOCK_SIZE) {
                ...
                for(auto& data : column[block]) {
                    for(auto& another_data : column[block]) {
                        if (another_data != data) {
                            bool data_insert = (data_pool[data]->m_neighbors.count(another_data) > 0);
                            bool another_data_insert = (data_pool[another_data]->m_neighbors.count(data) > 0);
                            assert(data_insert == another_data_insert );
                            if (!data_insert) {
                                data_pool[data]->m_neighbors.insert(another_data);
                                data_pool[another_data]->m_neighbors.insert(data);
                            }
                        }
                    }
                }
    ...
    for(auto& block : assigned_data_attr->m_blocks) {
        for(auto& data : column[block]) {
            if (data != assigned_data_attr->m_data && data_pool[data]->m_dict == -1) {
                ...
                assert(data_pool[data]->m_neighbors.size() == 0);
            }
        }
    }
    for(auto& data : assigned_data_attr->m_neighbors){
        if (data_pool[data]->m_dict == -1) {
            assert(data_pool[data]->m_neighbors.count(assigned_data_attr->m_data) > 0);
            data_pool[data]->m_neighbors.erase(assigned_data_attr->m_data);
        }
    }
    assigned_data_attr->m_blocks.clear();
    assigned_data_attr->m_freq = 0;
    assigned_data_attr->m_neighbors.clear();
}
...
int main (int argc, char *argv[]) {
    ...
    //std::cout<<"buid_data_time = "<<build_data_time<<"\n";//////////////
