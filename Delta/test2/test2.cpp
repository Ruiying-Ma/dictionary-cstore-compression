...
void CheckBuild(std::map<std::string, DataAttr*> &data_pool, std::vector<std::pair<std::string, std::set<std::pair<std::string, std::string>>>> &column, std::map<std::pair<std::string, std::string>, int> &data_pairs_pool) {
    for(auto& block : column) {
        assert(data_pool.count(block.first) > 0);
        for(auto& data_pair : block.second) {
            assert(data_pool.count(data_pair.first) > 0 && data_pool.count(data_pair.second) > 0 && data_pairs_pool.count(data_pair) > 0);
        }
    }
}

double ReadData(int data_set, std::map<std::string, DataAttr*> &data_pool, std::vector<std::pair<std::string, std::set<std::pair<std::string, std::string>>>> &column, std::map<std::pair<std::string, std::string>, int> &data_pairs_pool) {
    ...
    if (read_file.is_open()) {
        ...
        while (std::getline(read_file, data)) {
            ...
            if (offset == 0) {
                ...
                // body data
                // column
                std::string left = (prev_data < cur_data) ? prev_data : cur_data;
                std::string right = (prev_data < cur_data) ? cur_data : prev_data;
                std::pair<std::string, std::string> data_pair = std::make_pair(left, right);
                if ((left != right) && (column[block].second.count(data_pair) == 0)) {
                    column[block].second.insert(data_pair);
                    // data_pairs_pool
...
int Compute(std::map<std::string, DataAttr*> &data_pool, std::vector<std::pair<std::string, std::set<std::pair<std::string, std::string>>>> &column) {
    ...
    for(auto& block : column) {
        ...
        for(auto& data_pair : block.second) {
            assert(data_pool.count(data_pair.first) > 0 && data_pool.count(data_pair.second) > 0);
            int dict_first = data_pool[data_pair.first]->get_dict();
            int dict_second = data_pool[data_pair.second]->get_dict();
            assert(dict_first >= 1 && dict_first <= data_pool.size() );
            assert(dict_second >= 1 && dict_second <= data_pool.size());
            if (std::abs(dict_second - dict_first) > block_max) {
                block_max = std::abs(dict_second - dict_first);
            }
        }
        if (block_max == INT_MIN) {
            // no pairs in block.second
            assert(block.second.size() == 0);
            block_max = 0;
        }
        assert(block_max >= 0);
        if (block_max == 0){block_max = 1;}
        cost += (int)floor(log2((double)block_max) + (double)2) * (BLOCK_SIZE - 1) + (int)floor(log2((double)front_dict) + (double)2);
    }
    assert(cost >= 2 * BLOCK_NUM * BLOCK_SIZE);
    return cost;
}

int AssignByPairFreq(std::map<std::string, DataAttr*> &data_pool, std::map<std::pair<std::string, std::string>, int> &data_pairs_pool, std::map<int, OuterList> &outer_lists, std::vector<std::pair<std::string, std::set<std::pair<std::string, std::string>>>> &column) {
...
int AssignByRandom(std::map<std::string, DataAttr*> &data_pool, std::vector<std::pair<std::string, std::set<std::pair<std::string, std::string>>>> &column) {
...
int AssignByFrontFreq(std::map<std::string, DataAttr*> &data_pool, std::vector<std::pair<std::string, std::set<std::pair<std::string, std::string>>>> &column) {
    std::vector<std::pair<std::string, DataAttr*>> data_vec;
    std::copy(data_pool.begin(), data_pool.end(), std::back_inserter<std::vector<std::pair<std::string, DataAttr*>>>(data_vec));
    assert(data_vec.size() == data_pool.size());
...
int main(int argc, char *argv[]) {
    ...
    std::vector<std::pair<std::string, std::set<std::pair<std::string, std::string>>>> column;
